...
if (!(Test-Path $InputFile)) {
    throw "Codicon font file does not exist: $InputFile"
}
...
if ($bytes.Length -le 0) {
    throw "Codicon font file is empty: $resolvedInput"
}
...
[void]$sb.AppendLine("")
[void]$sb.AppendLine("inline constexpr unsigned char kCodicon[$($bytes.Length)] = {")
...
[void]$sb.AppendLine("};")
[void]$sb.AppendLine("")
[void]$sb.AppendLine("inline constexpr std::size_t kCodiconSize = sizeof(kCodicon);")

[System.IO.File]::WriteAllText($Output, $sb.ToString(), [System.Text.Encoding]::UTF8)

Write-Host "Embedded Codicon font:"
